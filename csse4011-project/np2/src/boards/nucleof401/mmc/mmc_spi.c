...
#define MMC_CS_LOW()       HAL_GPIO_WritePin(BRD_D4_GPIO_PORT, BRD_D4_PIN, 0)
#define MMC_CS_HIGH()      HAL_GPIO_WritePin(BRD_D4_GPIO_PORT, BRD_D4_PIN, 1)
...
//  	GPIO_PinAFConfig(NP2_MMC_SPI_MOSI_GPIO_PORT, NP2_MMC_SPI_MOSI_SOURCE, NP2_MMC_SPI_MOSI_AF);
	...
	GPIO_InitStructure.Alternate = BRD_SPI_SCK_AF;
...
//  	GPIO_Init(NP2_MMC_SPI_SCK_GPIO_PORT, &GPIO_InitStructure);
	...
	GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;			//Input Mode
	GPIO_InitStructure.Pull = GPIO_NOPULL;			//Enable Pull up, down or no pull resister
	GPIO_InitStructure.Alternate = BRD_SPI_MISO_AF;
...
//  	GPIO_Init(NP2_MMC_SPI_MISO_GPIO_PORT, &GPIO_InitStructure);
	...
	GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;
	GPIO_InitStructure.Pull = GPIO_PULLDOWN;
	GPIO_InitStructure.Alternate = BRD_SPI_MOSI_AF;
...
//  	GPIO_Init(NP2_MMC_SPI_MOSI_GPIO_PORT, &GPIO_InitStructure);
	...
	/* Set SPI Module */
//	SpiHandle.Instance = (SPI_HandleTypeDef *)(BRD_SPI);
	...
	SpiHandle.Instance = (SPI_TypeDef *)(SPI1_BASE);
...
//  	GPIO_Init(NP2_MMC_SPI_CS_GPIO_PORT, &GPIO_InitStructure);
	...
	HAL_GPIO_WritePin(BRD_D10_GPIO_PORT, BRD_D10_PIN, 1);
  	//GPIO_SetBits(NP2_MMC_SPI_CS_GPIO_PORT, NP2_MMC_SPI_CS_PIN);
	
	/* Configure GPIO PIN for MMC Power */
	// TODO Work out which pin is MMC power (Is this the MMC detect pin?)
...
/*-----------------------------------------------------------------------*/
/* @param buff is a pointer to the data to be sent						 */
/* @param bc the number of bytes to be sent								 */
/*-----------------------------------------------------------------------*/
static void xmit_mmc (const BYTE* buff, UINT bc) {
	...
	for (i = 0; i < bc; i++) {  // then write all unsigned char in buffer(*pBuf)
		...
		//while (HAL_SPI_GetState(&SpiHandle) != HAL_SPI_STATE_BUSY_TX); //TODO This flag seems wrong

		HAL_SPI_Transmit(&SpiHandle, &(buff[i]), 1, 1);

		dly_us(1000);
		//while (HAL_SPI_GetState(&SpiHandle) == HAL_SPI_STATE_BUSY);

		HAL_SPI_Receive(&SpiHandle, &rxbyte, 1, 1);
		dly_us(1000);

//		while (HAL_SPI_GetFlagStatus(NP2+, SPI_I2S_FLAG_TXE) == RESET);
//
...
//		dly_us(100);
		...
		HAL_SPI_Transmit(&SpiHandle, 0xFF, 1, 1);						// write dummy byte - must be non-command byte (0 to 58).
		while (HAL_SPI_GetState(&SpiHandle) != HAL_SPI_STATE_READY); 	// Wait for write to complete
		HAL_SPI_Receive(&SpiHandle, &rxbyte, 1, 1);						// Read unsigned char from SD Card
...
//		dly_us(100);
		...
		if ((i < 512) && ((i > 0) || (i == 0))) {
			HAL_SPI_Receive(&SpiHandle, &(buff[i]), 1, 1);
		}
...
void mmc_spi_test(){
	...
	uint8_t txByte = 0xDE;
	HAL_SPI_TransmitReceive(&SpiHandle, &txByte, &readbyte, 1, 10);
	txByte = 0xEA;
	HAL_SPI_TransmitReceive(&SpiHandle, &txByte, &readbyte, 1, 10);
	txByte = 0xBE;
	HAL_SPI_TransmitReceive(&SpiHandle, &txByte, &readbyte, 1, 10);
	...
	#ifdef DEBUG
		debug_printf("DEBUG: Sending test data over SPI (0xDEADBEEF)\n\n");
	#endif
}