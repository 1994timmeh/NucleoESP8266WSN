...
HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority) {
	...
	/* Compute the prescaler value */
  	PrescalerValue = (uint16_t) ((SystemCoreClock /2)/1000) - 1;		//Set clock prescaler to 50kHz - SystemCoreClock is the system clock frequency.

  	/* Time base configuration */
	TIM_Init.Instance = TIM5;				//Enable Timer 2
  	TIM_Init.Init.Period = 0x0000FFFF;			//Set period to be 1s
...
static UART_HandleTypeDef UART_debug;
...
void BRD_debuguart_init()
{
    ...
    UART_debug.Init.BaudRate   = 115200; //This has no effect apparently
	...
	/* Configure the GPIO RX pin for DEBUG UART */
...
//Transmit char through debug uart
void BRD_debuguart_putc(unsigned char c)
{
	//Transmit: *huart, *pData, Size, Timeout);
	HAL_UART_Transmit(&UART_debug, &c, 1, 1);
...
unsigned char BRD_debuguart_getc() {
	...
	if (__HAL_UART_GET_FLAG(&UART_debug, UART_FLAG_RXNE) == SET) {	
		c = __HAL_UART_FLUSH_DRREGISTER(&UART_debug);
		__HAL_UART_CLEAR_FLAG(&UART_debug, UART_FLAG_RXNE);
	}
	//HAL_UART_Receive(&UART_debug, &c, 1, 1);
	return c;
