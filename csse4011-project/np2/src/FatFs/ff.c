...
#define	ABORT(fs, res)		{ fp->flag |= FA__ERROR; LEAVE_FF(fs, res); }	//xxxxxxxxxxxxxxxxxxxxxxxx
...
{
	debug_printf("DEBUG: Attempting to open a file.\n");
	FRESULT res;
	DIR dj;
	BYTE *dir;
	DEF_NAMEBUF;
	...
	debug_printf("DEBUG: chk_mounted DSTATUS %d\n", res);
#else
	mode &= FA_READ;
	...
	if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
		...
		if (res != FR_OK) {					/* No file, create new */
			if (res == FR_NO_FILE)			/* There is no file to open, create a new entry */
				debug_printf("DEBUG: There is no file by that name... Creating a new one.\n");
#if _FS_SHARE
		...
		debug_printf("DEBUG: The file exists.\n");
...
)
{
	...
	//BYTE xbuff[512];	//xxxxxxxxxxxxxxxxxxxxxxxxxxxxx

//debug_printf("fread s%d\n\r", fp->buf[0]);
	/*debug_printf("fread c\n\r");
	if (disk_read(fp->fs->drv, fp->buf, 39646, 1) != RES_OK)	{// Fill sector cache 
		debug_printf("fread e\n\r");
	}	
	...
	for ( ;  btr;								// Repeat until all data read 
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		// On the sector boundary? 
			...
			if (cc) {							// Read maximum contiguous sectors directly 
				...
				debug_printf("fread a\n\r");
				if (disk_read(fp->fs->drv, rbuff, sect, (BYTE)cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
				debug_printf("fread b\n\r");
			...
			if (fp->dsect != sect) {			// Load data sector if not in cache 
				...
				//if (disk_read(0, xbuff, sect, 0) != RES_OK){debug_printf("fread e1\n\r");}	// Fill sector cache 
				//	ABORT(fp->fs, FR_DISK_ERR);
